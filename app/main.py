...
def classify_311_request(request_text: str) -> dict:
    ...
    # Parameters:
    #   model: AZURE_OPENAI_DEPLOYMENT (default "gpt-4o")
    #   messages: 
    #     - system: "Classify the following 311 service request into one of these categories: 
    #               Pothole, Noise Complaint, Trash/Litter, Street Light, Water/Sewer, Other"
    #     - user: request_text
    #   Expected response format:
    #     category: "Pothole" | "Noise Complaint" | "Trash/Litter" | "Street Light" | "Water/Sewer" | "Other"
    #     confidence: float between 0 and 1 indicating confidence in the classification
    #     reasoning: string with brief explanation of why the request was classified in that category
    
    response = client.chat.completions.create(
        model=os.environ.get("AZURE_OPENAI_DEPLOYMENT", "gpt-4o"),
        messages=[
            {"role": "system", "content": "Classify the following 311 service request into one of these categories: Pothole, Noise Complaint, Trash/Litter, Street Light, Water/Sewer, Other. Respond with a JSON object with keys: category (one of the categories), confidence (float between 0 and 1), reasoning (brief explanation of why the request was classified in that category)."},
...
def check_content_safety(text: str) -> dict:
    ...
    # Step 2.3 - Build categories dict and return safety results
    categories = {
...
def extract_key_phrases(text: str) -> list[str]:
    ...
    return response[0].key_phrases if response and len(response) > 0 else []
